...
swal({
 ...
 function() {
	        ...
	        $data["jscripts"] = array(
	        							...
	        							base_url() . "js/pages/encounters.js",
    ...
    function add_encounters(){
    	...
    	if($this->session->userdata("user_firstform") != 1)
    	{
		    ...
		    $config = array();
	        $config["base_url"] 	= base_url() . "encounters";
	        ...
	        if($this->session->userdata('filter_age_from')) 
	        {
		        ...
		        $users = $this->user_model->get_last_registered_users_like_page_with_param($config["per_page"], $page, $age_from, $age_to, $gender, $country, $city, $sort_by, $this->session->userdata('user_id'))->result_array();
    	
		        $users_count = $this->user_model->record_count_by_settings($age_from, $age_to, $gender, $country, $city);
		    } else {
	        	$users = $this->user_model->get_like_registered_users_page($config["per_page"], $page, $this->session->userdata('user_id'))->result_array();
				$users_count = $this->user_model->record_count();
		    	//echo '<pre>',print_r($users),'</pre>';exit();
